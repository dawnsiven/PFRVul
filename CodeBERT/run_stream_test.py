...
from torch.utils.data import DataLoader, Dataset, IterableDataset, SequentialSampler, RandomSampler,TensorDataset
...
def iter_json_array(file_path, chunk_size=1024 * 1024):
    ...
    with open(file_path, "r", encoding="utf-8") as handle:
        ...
        while True:
            ...
            if not buffer:
                if eof:
                    raise ValueError(f"Unexpected end of file while parsing {file_path}")
                continue
            ...
            if not buffer:
                if eof:
                    raise ValueError(f"Unexpected end of file after item in {file_path}")
                continue
            ...
            raise ValueError(f"Expected ',' or ']' while parsing {file_path}")
...
class StreamingTextDataset(IterableDataset):
    ...
    def __iter__(self):
        for js in iter_json_array(self.file_path):
            feature = convert_examples_to_features(js, self.tokenizer, self.args)
            index = js.get("index")
            if index is None:
                raise ValueError("Each test sample must include an 'index' field")
            yield (
                torch.tensor(feature.input_ids),
                torch.tensor(feature.label),
                torch.tensor(int(index)),
            )
...
def test(args, model, tokenizer, split="test"):
    eval_dataset = StreamingTextDataset(tokenizer, args, args.test_data_file)
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    ...
    logger.info("  Num examples = streaming")
    logger.info("  Batch size = %d", args.eval_batch_size)
    model.eval()
    ...
    with open(args.csv_path, "w", newline="", encoding="utf-8") as csv_handle:
        writer = csv.writer(csv_handle)
        writer.writerow(["Index", "Label", "Prediction", "prob"])
        ...
        for batch in tqdm(eval_dataloader, desc="Testing"):
            ...
            with torch.no_grad():
                batch_prob = model(inputs).detach().cpu().numpy()[:, 0]

            labels_np = label.numpy().astype(np.int32)
            preds_np = (batch_prob > 0.5).astype(np.int32)
            indices_np = indices.numpy().astype(np.int64)

            for index, label_value, pred_value, prob_value in zip(indices_np, labels_np, preds_np, batch_prob):
                writer.writerow([int(index), int(label_value), int(pred_value), float(prob_value)])

            total += len(labels_np)
            tp += int(np.sum((labels_np == 1) & (preds_np == 1)))
            tn += int(np.sum((labels_np == 0) & (preds_np == 0)))
            fp += int(np.sum((labels_np == 0) & (preds_np == 1)))
            fn += int(np.sum((labels_np == 1) & (preds_np == 0)))

    if total == 0:
        raise ValueError(f"No test samples were read from {args.test_data_file}")
